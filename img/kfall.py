...
for i in [1,2,3,4]:
	...
	spp1.append(spp)
	kf1.append(kf)
plt.title("Karp-Flatt measure vs number of processors")
plt.xlabel("Number of processors")
plt.ylabel("Karp-Flatt measure")
i=0
plt.ylim(ymin=0, ymax = 0.05)
color = ['red', 'blue', 'green', 'black']
for x in kf1:
	plt.plot(np[1:], x[1:], color = color[i], label = line_name[i])
...
plt.legend(bbox_to_anchor=(1.0, 1), loc=1, borderaxespad=0., handles=[red, blue, green, yellow])
plt.savefig("kfall.png")
