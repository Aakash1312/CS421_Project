...
for i in [1,2,3,4]:
	...
	for j in range(len(s1[i-1])):
		parallel_overhead = (j+1)*s1[i-1][j]-s1[i-1][0]
		psi = s1[i-1][0]/s1[i-1][j]
		eff = psi/(j+1) 
		sp.append(psi)
		spp.append(eff)
		ph.append(parallel_overhead)
		if j!=0:
	...
	kf1.append(kf)
	ph1.append(ph)
plt.title("Parallel Overhead vs number of processors")
plt.xlabel("Number of processors")
plt.ylabel("Parallel Overhead")
i=0
...
plt.legend(bbox_to_anchor=(0.5, 1), loc=1, borderaxespad=0., handles=[red, blue, green, yellow])
plt.savefig("phall.png")
